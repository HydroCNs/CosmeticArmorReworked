...
public class ContainerCosArmor extends Container {
    ...
    public ContainerCosArmor(InventoryPlayer invPlayer, InventoryCosArmor invCosArmor, EntityPlayer player) {
        ...
        // CraftingGrid
        for (int i = 0; i < 2; i++)
            for (int j = 0; j < 2; j++) addSlotToContainer(new Slot(craftMatrix, j + i * 2, 106 + j * 18, 26 + i * 18));
    ...
    public ItemStack transferStackInSlot(EntityPlayer player, int slotNumber) {
        ...
        if ((slot != null) && (slot.getHasStack())) {
            ...
            {
                if (!mergeItemStack(stack1, 13, 49, true)) return null;

                slot.onSlotChange(stack1, stack);
            } else if ((slotNumber >= 1) && (slotNumber < 5)) // CraftingGrid
            {
                if (!mergeItemStack(stack1, 13, 49, false)) return null;
            } else if ((slotNumber >= 5) && (slotNumber < 13)) // NormalArmor & CosmeticArmor
            {
                if (!mergeItemStack(stack1, 13, 49, false)) return null;
            } else if (((stack1.getItem() instanceof ItemArmor))
                && (!((Slot) inventorySlots.get(5 + ((ItemArmor) stack1.getItem()).armorType)).getHasStack())) {
            ...
            {
                if (!mergeItemStack(stack1, 40, 49, false)) return null;
            } else if ((slotNumber >= 40) && (slotNumber < 49)) // PlayerHotBar
            {
                if (!mergeItemStack(stack1, 13, 40, false)) return null;
